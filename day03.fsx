...
type Content = Set<int>
type Board = Map<Position, Content>
type Dir = | Up | Down | Left | Right | Unk

let getAt (b:Board) pos = Map.tryFind pos b |> Option.defaultValue Set.empty

let compileLine (inst:string) =
    inst.Split(',') 
    |> Seq.map (fun s -> 
...
let applyWire wireNum wire starting =
    let applyMove (board, p) (dir, arg) =
        ...
        |> Seq.fold (fun (b:Board, pos) _ -> 
            let loc = pos |> move dir
            b.Add(loc, (getAt b loc).Add(wireNum)), loc) (board, p)
    let board, _ = wire |> Seq.fold applyMove (starting, (0,0))
    board

let run program : Board =
...
let minIntersectionDist (board:Board) =
    board
    |> Map.toSeq
    |> Seq.filter (fun (_, v) -> Set.count v > 1)
    |> Seq.map (fun ((x,y), _) -> Math.Abs x + Math.Abs y)
    |> Seq.min
...
printfn "Day 3 Part 1 = %i" (run puzzle |> minIntersectionDist)

// Day 3 part 2 - for each intersection, determin total distance traveled by all the wires
// to get to that intersection.

let distToPos (wireId:int) (board:Board) (pos:Position) : int =
    -1
